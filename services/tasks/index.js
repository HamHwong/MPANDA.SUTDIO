...
class Tasks extends IService {
  ...
  async updateTaskFromArticle(Article) {
    ...
    let newTasks = tasks.filter(
      (task) =>
        !existedTasks.find(
          (existTask) => existTask.content.trim() === task.content.trim()
        )
    )
    newTasks = newTasks.filter((i, index) => newTasks.findIndex(j=>j.content.trim()===i.content.trim()) === index)
    let oldTasks = tasks.filter((task) =>
      existedTasks.find(
        (existTask) => existTask.content.trim() === task.content.trim()
      )
    ) 
    oldTasks = oldTasks.filter((i, index) => oldTasks.findIndex(j=>j.content.trim()===i.content.trim()) === index)
